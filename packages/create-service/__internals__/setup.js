...
const DEFAULT_NAME = '@microfrontend-example/service';
...
const _promptSchemaLibrarySuggest = {
  properties: {
    useSuggestedName: {
      description: colors.cyan(
        `Would you like it to be called '${suggestedLibraryName()}'? [Yes/No]`
      ),
      pattern: /^(y(es)?|n(o)?)$/i,
      type: 'string',
      required: true,
 ...
 * has been cloned into a custom directory (i.e. it's not '@microfrontend-example/service')
 *
 * @param {Error} err
 * @param {Response} res
 */
const suggestedLibraryNameAccept = () => {
  _prompt.get(_promptSchemaLibrarySuggest, (err, res) => {
    ...
    if (res.useSuggestedName.toLowerCase().charAt(0) === 'y') {
      setupLibrary(suggestedLibraryName());
    } else {
      libraryNameCreate();
    }
  });
...
/**
 * Checks if the suggested library name is the default, which is '@microfrontend-example/service'
 */
const suggestedLibraryNameIsDefault = () => {
  if (suggestedLibraryName() === DEFAULT_NAME) {
    return true;
  }
 ...
 */
const setupLibrary = (libraryName) => {
  console.log(
    colors.cyan(
 ...
 */
const modifyContents = (libraryName, username, usermail) => {
...
if (process.env.CI == null) {
  if (!suggestedLibraryNameIsDefault()) {
    suggestedLibraryNameAccept();
  } else {
    libraryNameCreate();
  }
} else {
