import { AnyImportedHook, BootHook, LoadHook, MountHook, RouteValidationFn, UnmountHook } from '../types';
...
export const registerApplication = <T = object>(name: string, loadFn: LoadHook<T>, routeValidFn: RouteValidationFn, customProps?: T): void => {
...
export const change = (eventArgs: Array<unknown>) => {
  ...
  const implementChanges = () => {
    return new Promise(async (resolve: Function, reject: Function) => {
      window.dispatchEvent(
        new CustomEvent('skeleton:before-change', createEvent(eventArgs))
      );
    });
  }
  ...
  const loadApps = async <T = object>() => {
    const promiseQueue = (apps as unknown as Array<App<T>>).map((app: App<T>): Promise<App<T>> => {
      return new Promise(async (resolve: Function): Promise<App<T>> => {
