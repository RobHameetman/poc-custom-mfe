...
export default class Registry {
  ...
  private static _events: RegistryEvents = {
    [Events.APP_CHANGE_STATUS]: <E extends AppChangeStatusEvent>(e: E): void =>
      Registry._changeStatus(...e.detail),
    [Events.APP_REGISTER]: <E extends AppRegisterEvent<T>, T = object>(e: E): void =>
      Registry._register(new App<T>(...e.detail)),
  };

  public static find(
    callback: (app: App, index: number, arr: Array<App>) => boolean
  ): App | undefined {
  ...
  public static asyncMap<U>(
    callback: (app: App, index: number, arr: Array<App>) => Promise<U>
  ): Array<Promise<U>> {
  ...
  private static _addEventListeners(): void {
    if (!this._listening) {
      Object.entries(
        this._events
      ).forEach(([type, handler]: [string, RegistryEventHandler]): void =>
        document.addEventListener(`core:${type}`, handler)
      );
    }
