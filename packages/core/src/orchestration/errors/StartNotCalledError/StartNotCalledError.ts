import { OrchestrationErrorCodes, OrchestrationErrors } from '../../enums';
import { Orchestrator } from '../../types';
import { StandardError, dispatch } from '../../../utils';

export const ERROR_START_NOT_CALLED = `${Orchestrator.namespace}:${OrchestrationErrors.StartNotCalled}`;
export type ERROR_START_NOT_CALLED = typeof ERROR_START_NOT_CALLED;

export class StartNotCalledError
  extends Error
  implements StandardError<ERROR_START_NOT_CALLED, OrchestrationErrorCodes.StartNotCalled> {
...
export const isStartNotCalledErrorEvent = (value: unknown): value is StartNotCalledErrorEvent => {
  return (
    typeof value === 'object' &&
    value !== null &&
    'type' in value &&
